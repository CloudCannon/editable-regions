...
export default function (eleventyConfig, pluginOptions) {
  ...
  eleventyConfig.on("eleventy.before", async () => {
    ...
    const extensions = normalizeToArray(pluginOptions.liquid?.extensions, [".liquid", ".html"]);
    const finalExtensions = [...new Set(extensions.map(ext => {
    ...
    await esbuild.build({
      ...
      outfile:
        ...
        `${eleventyConfig.dir.output}/live-editing.js`,
    });
  });
}
...
const createLiveEditingSource = async (pluginOptions) => {
  ...
  if (pluginOptions.liquid) {
    const componentDirs = normalizeToArray(pluginOptions.liquid.component_dirs, ["src/_includes/"]);
    const extensions = normalizeToArray(pluginOptions.liquid.extensions, [".liquid", ".html"]);
