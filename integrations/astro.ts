...
interface Renderer {
  ...
  ssr: {
    ...
    renderToStaticMarkup: (Component: any, props: any, slots?: any) => Promise<string | { html: string }>;
  };
}
...
const renderers: Renderer[] = [
  {
    ...
    ssr: {
       ...
       */
      renderToStaticMarkup: async (Component: string, props: Record<string, any>, slots: Record<string, string>): Promise<string> => {
        const propsString = Object.entries(props)
          .map(([key, value]) => `${key}="${value}"`)
          .join(" ");
        return `<${Component} ${propsString}>${slots.default ?? ""
          }</${Component}>`;
      },
    },
  },
  {
    ...
    ssr: {
       ...
       */
      renderToStaticMarkup: async (Component: FunctionComponent<any>, props: any): Promise<{ html: string }> => {
        try {
 ...
 */
export const registerAstroComponent = (key: string, component: Function): void => {
  /**
   ...
   */
  const wrappedComponent = async (target: HTMLElement, props: any): Promise<void> => {
    /**
