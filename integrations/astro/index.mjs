 ...
 */
export const queueForClientSideRender = (
  renderFunction
) => {
...
export const registerAstroComponent = (key, component) => {
  ...
  const wrappedComponent = async (props) => {
    ...
    doc
      .querySelectorAll("[data-editable-region-csr-id]")
      .forEach((node) => {
        const csrId = Number(
          node.getAttribute("data-editable-region-csr-id")
        );
    ...
    doc
      .querySelectorAll("link, [data-island-id]")
      .forEach((node) => {
        node.remove();
      });
