 ...
 * are rewritten to `.json` so Hugo decodes them natively. The delete path uses
 * the same rewritten target, so removals stay in sync with the mirror.
 */
...
test(".yaml/.json datasets keep their extension; others are rewritten to .json", async () => {
	// Trigger a render so the WASM engine (and its boot-time dataset mirror)
	// has finished before we inspect the mirrored files.
	...
	// A .toml source is mirrored as .json (not .toml) with JSON contents.
