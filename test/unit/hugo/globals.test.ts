/**
 * What the globals components can reach at render time: the `site` global
 * object (a real Hugo `Site`), the `hugo.*` namespace, and query helpers over
 * site data. Runs the full consumer chain — fixture built by real Hugo,
 * shipped in the live-editing bundle, written into the WASM renderer by the
 * browser runtime.
 */
...
// The fixture's data/ files, mirrored at boot as datasets (the data source;
// data files are no longer part of the template snapshot).
